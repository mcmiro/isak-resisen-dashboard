...
const OrderCard = ({ vehicles }: OrderCardProps) => {
  ...
  const [currentOrders, setCurrentOrders] = useState<OrderModelWithId[]>();

  function groupOrdersByDate(orders: OrderModelWithId[]) {
        ...
        Object.keys(groupedOrders).map((date) => (
          <div key={date}>
            ...
            >
              ...
              <ul>
                {groupedOrders[date].map(
                  (order: OrderModelWithId, index: number) => (
                    <div
                      key={index}
